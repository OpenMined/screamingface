...
| `url4-engine` | Implementing or reviewing URL4 execution, `Url4Node`, `url4 serve`, or an application-owned node profile | Separates the implemented GET-only node/CLI/registry contracts from proposed streaming and telemetry architecture. ScreamingFace profile guidance covers the temporary app under `packages/screamingface/apps/`. |
