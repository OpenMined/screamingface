...
A compatible ScreamingFace engine is one persistent `Url4Node` application with
Python-registered handlers. Its deployable implementation is not part of the ScreamingFace SDK
package. The application owns:
...
Do not force an application profile into `url4.toml`: Python handlers are appropriate when a route needs
...
- ScreamingFace SDK change: run the complete `screamingface` package stack, including deterministic
  notebook and fixture checks.
- ScreamingFace engine change: use the engine owner's independent application lane.
