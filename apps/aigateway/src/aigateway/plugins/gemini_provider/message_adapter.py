...
def model_response_from_gemini(data: dict[str, Any], fallback_model: str) -> ModelResponse:
    ...
    if not isinstance(candidates, list) or not candidates:
        raise CustomLLMError(status_code=502, message="Gemini response missing candidates")
    candidate = candidates[0]
    if not isinstance(candidate, dict):
        raise CustomLLMError(status_code=502, message="Gemini candidate is not an object")
    ...
    return ModelResponse(
        id=data.get("traceId") or f"chatcmpl-{uuid.uuid4().hex}",
