...
``MissingCredentials`` is raised in
:func:`screamingface_engine.auth.dependencies._extract_capability` when a request
carries no capability header at all;
:class:`screamingface_engine.auth.jwt.JwtCodec` never sees that case. The other
four subclasses are raised by :meth:`screamingface_engine.auth.jwt.JwtCodec.verify`
for the ways a present token can fail verification.
`dependencies.verified_claims` catches ``AuthError`` and translates it into a 401
RFC 9457 problem response, so these are internal signals, not something clients see
directly.
