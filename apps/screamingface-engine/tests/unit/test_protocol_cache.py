...
INVARIANT (spec §1.0): aigateway v2's cache-control grammar is **CLOSED to exactly
one field**, `use-cache`. Any other key inside a request body's `cache` object makes
the WHOLE request **bypass** — silently, with no error anywhere, even alongside a
valid `use-cache: true`. So this protocol type is `extra="forbid"`: a caller
inventing `no_store` fails loudly HERE, at the url4 edge, instead of having it
forwarded to a grammar whose only answer is to quietly cost every hit.

The type carries **intent only** (`participate`); the translation into aigateway's
wire vocabulary lives in `apps/screamingface-engine` (plan §4), because
`packages/url4` ships to SDK users and must not know an adapter's request-body
shape. `max_age` is url4-internal and is **never** sent upstream — v2 refuses it
(`global_controls.py:79-83`).
...
def test_cache_policy_is_exported_from_the_protocol_package() -> None:
    """screamingface-engine imports the protocol as a package; a type reachable
    only by module path is
