...
**Both ends of that trip are the same image.** `apps/url4-cloud` ships one distribution
(`url4_cloud`) and one image (`ghcr.io/openmined/screamingface-url4-cloud`) with two modes
selected by argv: `url4-cloud serve` is the App, `url4-cloud run` is the Job. The halves are kept
...
| Runner Job | `Job` (run‑once: `backoffLimit:0`, `restartPolicy:Never`, `activeDeadlineSeconds`) | **The App's own image**, `command: ["url4-cloud", "run"]`. Evaluates the url4 expression, publishes the lifecycle, exits. |
...
sequenceDiagram
    ...
    participant App as url4-cloud App<br/>(url4-cloud serve)
    ...
    participant Runner as Runner Job Pod<br/>(same image, url4-cloud run)
    ...
    Runner->>Runner: cli.main(["run"]) → lazily imports url4_cloud.runner.main
...
- **One image, two modes — the mode comes from argv.** The Job runs the App's own image with
  `command: ["url4-cloud", "run"]` (pinned in `adapters/k8s.py`), so the two can never be at
...
| credential hop | `rest/routes.py::_forwarded_credential`, `adapters/k8s.py::_env`, `url4_cloud/runner/main.py::build_executor` (names from the single `job_env.py` — a shared leaf, no longer a hand‑synced pair) |
