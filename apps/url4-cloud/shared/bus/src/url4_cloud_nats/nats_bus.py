...
class NatsBus:
    ...
    async def subscribe(
        ...
        # WebSocket to attach before the run starts, yet the stream is only created when the
        # Runner first publishes. Binding first raises NotFoundError, and because ``Bridge._pump``
        # is a bare task with no done-callback, that failure is silent: the client then sees
        # heartbeats forever.
