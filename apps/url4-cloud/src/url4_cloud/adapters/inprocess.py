...
# INVARIANT: engine-free. The concrete `Executor` arrives through an injected factory, so this
# module imports neither the url4 engine nor `url4_cloud.runner`. That is what keeps it a SHARED
# leaf under `.claude/scripts/check_layering.py` rather than a hole in the control-plane/run-mode
# boundary — the one module that fuses the two halves is `url4_cloud.local`, and it is exempt
# there by name.
"""
