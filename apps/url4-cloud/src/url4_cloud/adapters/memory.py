...
Same port, same observable semantics (1-based sequence numbers, replay-from-sequence, purge), no
broker: the log is a dict of per-topic lists and readers wake on an `asyncio.Condition`. This is
what makes `--local` a real deployment rather than a mock — the REST sync-hold and the WS pump
both subscribe to it exactly as they subscribe to `JetStreamConsumer`.

It also remains the fake the test suite runs against (re-exported by `url4_cloud.testing`), which
is the reason for the parity note below.
