...
prod, ``docker`` for the local/compose e2e — both take an injected client so tests run headless.
"""
...
# k8s recommended labels (docs/protocol.md §9) — also stamped on the docker container.
RUNNER_LABELS = {
...
class JobRunner(Protocol):
    ...
    - ``schedule(topic, url4, deadline_s)`` — create the run-once job for ``topic`` carrying the
      url4 expression; returns the deterministic :func:`job_name`. Raises :class:`JobAlreadyExists`
      when a job for ``topic`` already exists.
