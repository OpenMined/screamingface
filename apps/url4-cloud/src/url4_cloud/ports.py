"""url4-cloud's local widening of the ports it does not own.

`url4.streaming.interfaces.JobRunner` is the abstract substrate port, and it lives in the engine
distribution: it describes scheduling a run in terms every substrate shares. Carrying the CALLER's
verified identity to aigateway is not one of those terms — it is this app's deployment concern (see
:data:`url4_cloud.job_env.IDENTITY_HEADER_ENV`), and aigateway is not part of the url4 language.

So the parameter is declared HERE rather than on the engine's port. Adding an optional
keyword-only argument in an override widens the contract — every caller written against the base
port still type-checks — which is what lets the two adapters accept identity without
`packages/url4` gaining a concept that only this app has.

A shared leaf by construction: it imports nothing from `url4_cloud` beyond the abstract port, so
both the control plane and the run mode may import it (`.claude/scripts/check_layering.py`).
"""
...
class IdentityAwareJobRunner(JobRunner):
    ...
    run's environment with :func:`url4_cloud.job_env.identity_to_env`; the Runner reads it back and
    renders it onto its outgoing aigateway request.

    WHY it is not simply forwarded as headers: the App that receives them and the Runner Pod that
