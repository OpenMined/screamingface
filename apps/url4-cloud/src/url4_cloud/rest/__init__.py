"""The url4-cloud REST surface: run lifecycle, token minting, and catalog routes.

Re-exports the FastAPI ``APIRouter``s (``router`` for run/token endpoints, ``catalog_router``
for the model catalog, and ``benchmark_router`` for installed benchmark IDs) and the
