...
def test_run_resolves_the_real_runner_entrypoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The tests above replace `cli._run` wholesale, which would keep passing even if its import
    line named a module that no longer exists. This one patches the far side instead, so a
    rename under `url4_cloud.runner` fails here rather than at a Job's first boot in-cluster.
    """
    import url4_cloud.runner.main as runner_main
...
def test_run_mode_does_not_import_the_serving_stack() -> None:
    ...
    """
    probe = (
        "import sys, url4_cloud.runner.main;"
