...
def _failure_codes(result: dict) -> dict[int, str | None]:
...
def test_fully_judged_cases_score_and_mean_unclipped(tmp_path: Path) -> None:
    ...
    assert first["grade"]["method"] == "rubric"
    assert first["grade"]["score"] == pytest.approx(-0.4)
    assert [check["id"] for check in first["grade"]["checks"]] == ["1", "2", "3"]
    assert first["grade"]["checks"][2]["evidence"][0]["outcome"] == "MET"
...
def test_partial_verdicts_never_score(tmp_path: Path) -> None:
    ...
    assert len(result["cases"][0]["grade"]["checks"]) == 1
