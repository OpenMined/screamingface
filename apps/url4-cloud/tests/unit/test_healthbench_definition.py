...
def test_both_exams_are_registered_under_their_ids() -> None:
    assert BENCHMARKS["healthbench/worst30"] is HEALTHBENCH_WORST30
    assert BENCHMARKS["healthbench/smoke"] is HEALTHBENCH_SMOKE
...
def test_the_subset_is_the_frozen_157() -> None:
    ...
    assert HEALTHBENCH_WORST30.case_ids == WORST30_CASE_IDS
    assert HEALTHBENCH_SMOKE.case_ids == (SMOKE_CASE_ID,)
...
def test_the_candidate_is_invoked_without_retrieval() -> None:
    rendered = _url4(HEALTHBENCH_WORST30)
    assert "/candidate?web_search=false" in rendered
