...
The engine is a separately owned deployment. This package contains only the researcher SDK and its
public wire-contract fixtures; it does not contain or start an engine implementation.

## Quickstart

With a compatible ScreamingFace engine available, use the SDK:
...
## Expected engine benchmark scope

The current engine contract advertises `gpqa@1` plus DRACO when its pinned OpenRouter judge appears in
