# screamingface-engine — reference node profile

This app is an implementation handoff for the ScreamingFace URL4 node profile. It remains
temporarily under `packages/screamingface/apps/`, is not included in the `screamingface` wheel,
and is not SDK-owned production infrastructure. The engine owner can promote it to
`apps/screamingface-engine/`, incorporate its `create_node(...)` factory into a runner, or retain
it only as an executable contract reference without changing the public HTTP contract.
...
```bash
cp env.example .env
# Edit .env locally and set HF_TOKEN=hf_...; never commit it.
./dev.sh restart
```
...
Rebuild after changing either this app or `packages/url4`; Docker images are snapshots and do not
pick up source changes until `./dev.sh restart` completes.
