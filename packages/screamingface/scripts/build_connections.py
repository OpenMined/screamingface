...
Have a compatible ScreamingFace engine available. This SDK package does not bundle or start it.
...
means the engine validated and retained the Tavily credential according to its deployment policy.
Neither status claims that every advertised model or future tool action will succeed."""
        ),
...
examples are comments so running this guide never starts or replaces a connection unexpectedly."""
        ...
        nbformat.v4.new_code_cell(
            ...
            "# Tavily is validated and retained by the configured engine:\n"
...
Dataset access remains separate from inference-provider connections. A compatible engine obtains
its dataset credential from its own deployment environment when a registered benchmark must load a
gated Hugging Face source; `sf.connect()` does not manage that token. Tavily credentials are
engine-owned and never pass through AI Gateway."""
        ),
    ]
