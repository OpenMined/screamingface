...
class _NotebookConnectionView:
        ...
        if "oauth" in connection.auth_methods:
            ...
            oauth.on_click(
                lambda _: self._controller._attempt(
                    lambda: self._controller._set_notice(
                        "OAuth is not advertised by this Engine yet."
                    )
                )
            )
    ...
    def _oauth_controls(self, connection: Connection) -> list[Any]:
        ...
        cancel.on_click(lambda _: self._controller._cancel_flow(connection.provider))
        return [link, cancel]
