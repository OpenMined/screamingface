...
def test_majority_recipe_is_canonical_parameterized_and_network_free() -> None:
    ...
    assert "member_1:0.0:/codex/gpt-5.5($question)!'Answer the question.'" in recipe
    assert "/gemini/2.5-flash?temperature=0.2&enabled=true&q=($question)" in recipe
    assert "recipe_answer:0.0:/reducers/majority-vote/1()!'$member_answers'" in recipe
...
def test_concrete_expression_binds_literal_question_without_a_reference() -> None:
    ...
    assert "question:0.0:'What does $$5 buy?'" in expression
...
def test_multiline_prompts_and_questions_use_url4_safe_line_separators() -> None:
    ...
    assert build(expression)
    assert "question:0.0:'Question line one\u2028Question line two'" in expression
...
def test_benchmark_tools_compile_only_onto_members_and_round_trip_through_url4() -> None:
    ...
    assert "recipe_answer:0.0:/reducers/majority-vote/1()!'$member_answers'" in expression
    assert "fusion_answer:0.0:/reducers/majority-vote?" not in expression
...
def test_official_benchmark_compiles_one_shared_versioned_tool_policy() -> None:
    ...
    assert build(expression)
    assert expression.count("tool_policy:0.0:/benchmarks/research/1/tool-policy") == 1
    ...
    assert "member_1:0.0:/codex/gpt-5.5($model_input)" in expression
    assert "member_2:0.0:/gemini/2.5-flash($model_input)" in expression
...
def test_model_reducer_receives_automatic_labeled_context_and_its_own_intent() -> None:
    ...
    assert "recipe_answer:0.0:/codex/gpt-5.5?temperature=0.0&q=(question=$question" in recipe
    assert "fusion_answer:0.0:/codex/gpt-5.5?tools=" not in recipe
    assert "panel_1={model: 'codex/gpt-5.5', answer: '$member_1'}" in recipe
    assert "panel_2={model: 'gemini/2.5-flash', answer: '$member_2'}" in recipe
...
def test_model_expression_accepts_default_parameters() -> None:
    ...
    assert build(expression)
    assert "model_result:0.0:/codex/gpt-5.5($model_context)!'Answer'" in expression
