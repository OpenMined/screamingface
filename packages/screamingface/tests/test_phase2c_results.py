...
def test_successful_run_is_immutable_typed_and_json_compatible() -> None:
    ...
    second = sf.MemberResult("gemini/2.5-flash", "B")
    ...
    assert run.recipe_name == "tiny-fusion"
    assert run.members == {
    ...
    assert run.to_dict()["recipe_name"] == "tiny-fusion"
...
def test_failed_result_is_atomic_and_surfaces_a_standalone_failure() -> None:
    ...
    run = sf.Run(
        ...
        recipe_name="tiny-fusion",
        recipe_url4="(recipe)",
        members={"member_1": "codex/gpt-5.5", "member_2": "gemini/2.5-flash"},
        cases=benchmark._materialize_cases(),
        results=[result],
    )
