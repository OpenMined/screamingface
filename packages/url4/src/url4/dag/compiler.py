...
def _lower_collection(node: Node, edges: Edges, registry: LoweringRegistry) -> DagNode:
    ...
    ``edges`` are the enclosing group's sibling bindings, wired through so a
    reference in collection position (``$gate*(body)``) resolves exactly like the
    same reference in body position. They used to be dropped here (``{}``), which
    made a sibling-named collection resolve to its literal ``$name`` text and fail
    as "scalar, not an iterable collection" — a reference the grammar admits was
    silently unusable in exactly one slot. Deterministic 0-or-1-item gate
    collections (the conditional-execution idiom) depend on this wiring."""
